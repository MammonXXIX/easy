...
import { Bookmark, Calendar, Flame, MessageCircle } from "lucide-react";
...
const ReadPage: NextPageWithLayout<InferGetStaticPropsType<typeof getStaticProps>> = ({ id }) => {
  ...
  const { mutate: updateSavePost } = trpc.post.updateSavedPost.useMutation({
    onMutate: async ({ id }) => {
      await trpcUtils.post.getUserPostPublic.cancel({ id })

      const prevData = trpcUtils.post.getUserPostPublic.getData({ id })

      trpcUtils.post.getUserPostPublic.setData({ id }, (oldData) => {
    ...
    onError: (_err, _variables, context) => {
      if (context?.prevData) {
        trpcUtils.post.getUserPostPublic.setData({ id }, context.prevData)
      }
    },
    onSettled: () => {
      trpcUtils.post.getUserPostPublic.invalidate({ id })
    },
  }) 
      ...
      {post && (
        <div className="max-w-4xl flex flex-col">
          <div className="p-4 space-y-4">
            ...
            <div className="p-4 flex justify-between items-center border-y-2">
              <div className="flex items-center gap-6">
                <span className="flex items-center gap-1 text-sm cursor-pointer"><Flame size={20} /> 600</span>
                <span className="flex items-center gap-1 text-sm cursor-pointer"><MessageCircle size={20} /> 60</span>  
              </div>
              <div className="flex items-center gap-2">
                <span onClick={() => updateSavePost({ id })} className="cursor-pointer">
                  {
                    post.isSaved
